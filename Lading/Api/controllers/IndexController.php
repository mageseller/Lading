...
class Lading_Api_IndexController extends Mage_Core_Controller_Front_Action {
	function getTreeCategories($parentId, $isChild){
	    ...
	    foreach ($allCats as $category) 
	    {
	    	...
	    	$child = array();
	        if($subcats != ''){
	            $child =  $this->getTreeCategories($category->getId(), true);
	        }
	    	$_categorylist[] = array(
				...
				'url_key' => Mage::getModel('catalog/category')->load($category->getId())->getUrlPath(),*/
				'thumbnail_url' => Mage::getModel('catalog/category')->load($category->getId())->getThumbnailUrl(),
				/*'image_url' => Mage::getModel('catalog/category')->load($category->getId())->getImageUrl(),*/
				'child'	=> $child ,
			);
	...
	public function indexAction() {
		...
		switch ($cmd) {
			case 'menu' : // OK MENU
					...
					$_categorylist = $this->getTreeCategories($rootCatId, false);
					echo json_encode (array('code'=>0, 'msg'=>null ,'model'=>$_categorylist),JSON_PRETTY_PRINT);
				// ---------------------------------列出产品目录 END----------------------------------------//
				break;
