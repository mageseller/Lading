    ...
    public function getProductVariations($productId){
                      ...
                      /* $products['collection'][] = array(
                        ...
                        foreach($collection as $key=>$exist_collection) {
                            if ((int) $child->getId() == $exist_collection['id']){
                                ...
                                $productData =  Mage::getModel('catalog/product')->load((int) $temp_collection['id']);
                                $temp_image_url = $productData->getImageUrl ();  //add by wayne
                                $temp_collection['child_image_url'] = $temp_image_url;
                                $price = $price = Mage::getModel('mobile/currency')->getCurrencyPrice(($productData->getSpecialPrice()) == null ? ($productData->getPrice()) : ($productData->getSpecialPrice()));
                				$price = number_format($price, 2, '.', '' );  
                                $temp_collection['qty'] = array("qty" => (int)Mage::getModel('cataloginventory/stock_item')->loadByProduct($productData)->getQty(), "price" => $price)  ; 
        ...
        foreach($collection as $after_collection){
            ...
            array_push($temp_image_item,$after_collection['child_image_url']);  //add by wayne
             array_push($temp_image_item,$after_collection['qty']);
...
//            }
    ...
    public function getProductLabelsKey(Mage_Catalog_Model_Product $product) {
	    ...
	    foreach ($values as $value) {
	    	$optionColor[$value['value']] = $value['label'];
	    }
        ...
        foreach ($productAttributeOptions as $productAttribute) {
            ...
            foreach ($productAttribute['values'] as $attribute) {
               // $attributeOptions[$productAttribute['label']][$attribute['value_index']] = $attribute['store_label'];
            	 $attributeOptions[$productAttribute['label']][$i]['value_index'] =$attribute['value_index'];
                 $attributeOptions[$productAttribute['label']][$i]['store_label'] = $attribute['store_label'];
                 $attributeOptions[$productAttribute['label']][$i]['store_label1'] =$optionColor[$attribute['value_index']];
                 $i++;
            }
        }
    ...
    public function getProductCustomOptionsOption($product){
	
		$i = 0;
	
		foreach ($product->getOptions() as $o) {
			
			$customOptionGroup[$i]['custom_option_type'] = $o->getType();
			$customOptionGroup[$i]['custom_option_title'] = $o->getTitle();
            $customOptionGroup[$i]['is_require'] = $o->getIsRequire();
            $customOptionGroup[$i]['option_id'] = $o->getId();
			
			// Getting Values if it has option values, case of select,dropdown,radio,multiselect
			$values = $o->getValues();
			foreach ($values as $v) {
				$customOptionValues[] = $v->getData();
            }

			$customOptionGroup[$i]['custom_option_value'] = $customOptionValues;
			$i++;
